...
import { 
  LayoutDashboard, LogOut, Search, Bell, Calendar,
  ChevronRight, X, Clock, GraduationCap, Award, User
} from 'lucide-react';

export default function StudentLayout({ children, activePage, headerTitle, headerActions }) {
  ...
  const searchItems = [
    { label: 'Student Dashboard Overview', path: '/', category: 'Page', desc: 'Home metrics and assigned evaluations', keywords: ['home', 'stats', 'overall', 'dashboard'] },
  ];
  ...
  const sidebarItems = [
    ...
    { name: 'Available Exams', path: '/?tab=exams', icon: GraduationCap },
    { name: 'Certificates', path: '/?tab=certificates', icon: Award },
    { name: 'Exam Log', path: '/?tab=log', icon: Clock },
    { name: 'My Profile', path: '/?tab=profile', icon: User },
  ];
        ...
        <div className="px-5 pt-7 pb-5 flex items-center justify-between gap-3 select-none">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[9px] text-indigo-300 font-bold tracking-widest block mt-0.5">STUDENT PORTAL</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-slate-400 text-[10px] font-semibold mb-0.5 hidden sm:block">
                Hello {user?.name?.split(' ')[0] || 'Student'}, Welcome back
              </p>
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
              ...
              <div className="relative flex-shrink-0" data-panel>
                ...
                >
                  ...
                  {notifications.some(n => !n.read) && (
                    <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_4px_rgba(99,102,241,0.8)]" />
                  )}
                </button>
                  ...
                  <div className="fixed top-16 left-4 right-4 md:absolute md:right-0 md:left-auto md:top-11 md:w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-fade-in" data-panel>
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
                      ...
                      <input
                        ...
                        placeholder="Search student options..."
                    ...
                    <div className="p-4">
                      {searchQuery ? (
                          ...
                          <div className="flex flex-col gap-1.5 max-h-64 overflow-y-auto scrollbar-thin">
                            ...
                            {filteredItems.map((item, idx) => (
                              ...
                              >
                                ...
                                <span className="text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ml-3 bg-indigo-50 border border-indigo-100 text-indigo-600">
                                  {item.category}
                                </span>
                              </button>
                            ))}
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <button
                            onClick={() => { navigate('/'); setShowSearch(false); }}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-indigo-50 text-left transition-colors group"
                          >
                            <LayoutDashboard className="w-3.5 h-3.5 text-indigo-400 group-hover:text-indigo-600 flex-shrink-0" />
                            <span className="text-xs font-bold text-slate-600 group-hover:text-indigo-700">Student Dashboard</span>
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <div className="relative flex-shrink-0" data-panel>
              ...
              <button
                ...
                title={user?.name || 'Student Candidate'}
              >
              ...
              {showProfileMenu && (
                <div className="fixed top-16 left-4 right-4 md:absolute md:right-0 md:left-auto md:top-11 md:w-52 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-fade-in" data-panel>
                  ...
                  <div className="px-4 py-3 bg-gradient-to-r from-indigo-50 to-indigo-50/30 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-black text-slate-800 truncate">{user?.name || 'Student Candidate'}</p>
                      <p className="text-[9px] text-indigo-600 font-bold mt-0.5 truncate">{user?.batch || 'Web Development'}</p>
                      <p className="text-[8px] text-slate-400 font-medium truncate">{user?.email}</p>
                    </div>
                  </div>
                  <button
